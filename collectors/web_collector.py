...
class WebCollector(BaseCollector):
    ...
    async def collect(self, **kwargs) -> CollectorResult:
        ...
        max_per_site = kwargs.get('max_per_site', 10)

        for site in self.sites:
            try:
                logger.info(f"采集网站: {site['name']}")
                items = await self._collect_site(site, max_per_site)
        ...
        return CollectorResult(
            ...
            message=f"成功采集 {success_count}/{len(self.sites)} 个网站，过滤后 {len(filtered_items)}/{len(all_items)} 条",
    ...
    def _parse_markdown_content(self, markdown_text: str, site_config: Dict, max_items: int) -> List[RawIntelData]:
        ...
        # Markdown 中的链接模式: [标题](URL)
        link_pattern = re.compile(r'\[([^\]]+)\]\((https?://[^\)]+)\)')
        matches = link_pattern.findall(content)

        for title, url in matches[:max_items]:
            # 过滤非新闻链接（导航、footer等）
            if any(skip in url.lower() for skip in ['javascript:', '#', 'tel:', 'mailto:']):
                continue
            if any(skip in title.lower() for skip in ['skip to', 'copyright', 'privacy', 'terms']):
                continue
            ...
            title_pos = content.find(f'[{title}]({url})')
            if title_pos > 0:
                # 取链接前后的文本作为上下文
